...
class ElfLoader : Loader
{
  ...
  void load(Document prog, string path)
  {
    ...
    // map sections
    foreach(raw_sect; elf.raw_sections)
    {
      const sh = &elf.section_header[raw_sect.section_index];

      // skip reserved sections : they will be generated by the assembler
...
private:
  ...
  /**
   * @brief Searches for a symbol corresponding to location indicated by (SectionIndex, Offset)
   *
   * @param SectionIndex the section containing the location.
   * @param Offset the offset of the location, relative to the beginning of the section.
   *
   * @return the name of a symbol corresponding to the location.
   */
  string SearchSymbol(SECTION_IDX LocSectionIdx, uint LocOffset)
  {
    foreach(SymbolTable; symbol_tables)
    {
      SECTION_IDX SymTabSectionIdx = SymbolTable.section_index;

      foreach(SYM_TAB_IDX SymbolIdx, SymbolEntry; SymbolTable.symbols)
      {
        if(SymbolIdx == 0)
          continue;

        if(SymbolEntry.st_shndx == SHN_ABS)
          continue;

        if(SymbolEntry.st_value == LocOffset && SymbolEntry.st_shndx == LocSectionIdx)
        {
          string SymbolName = GetSymbolName(SymTabSectionIdx, SymbolIdx);
          assert(SymbolName != "");
          return SymbolName;
        }
      }
